 ...
 * @author Medard
 */
public class TechnicienBlockDAOImpl implements TechnicienDAO {
    ...
    public Technicien createTechnicien(Technicien technicien) {
        ...
        try {
            ...
            String sql = "INSERT INTO aphrodis.technicien_block(nom,prenom,tel) VALUES (?,?,?)";
    ...
    public boolean deleteTechnicien(int id) {
        ...
        try {
            conn.setAutoCommit(false);
            String sql = "DELETE FROM aphrodis.technicien_block WHERE id = \'" + id + "\'";
            pstmt = conn.prepareStatement(sql);
            pstmt.executeUpdate();
            conn.commit();
            conn.setAutoCommit(true);
    ...
    public List<Technicien> getAllTechniciens() {
        ...
        Connection conn = new ConnectionInit().getCon();
        try {
            String sql = "SELECT * FROM aphrodis.technicien_block";
    ...
    public boolean updateTechnicien(Technicien technicien) {
        ...
        try {
            conn.setAutoCommit(false);
            String sql = "UPDATE aphrodis.technicien_block SET nom = ?, prenom= ?, tel = ? WHERE id = ?";
            pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, technicien.getLastName());
            pstmt.setString(2, technicien.getFirstName());
            pstmt.setString(3, technicien.getNumTel());
    ...
    public List<Technicien> getTechnicienByLastName(String lastName) {
        ...
        Connection conn = new ConnectionInit().getCon();
        try {
            String sql = "SELECT * FROM aphrodis.technicien_block WHERE nom = \'" + lastName + "\'";
            rs = conn.createStatement().executeQuery(sql);
            while (rs.next()) {
                technicien = new Technicien(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4));
    ...
    public List<Technicien> getTechnicienByFirstName(String firstName) {
        ...
        Connection conn = new ConnectionInit().getCon();
        try {
            String sql = "SELECT * FROM aphrodis.technicien_block WHERE prenom = \'" + firstName + "\'";
            rs = conn.createStatement().executeQuery(sql);
            while (rs.next()) {
                technicien = new Technicien(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4));
                techs.add(technicien);
    ...
    public Technicien getTechnicienByPhoneNum(String phoneNum) {
        ...
        Connection conn = new ConnectionInit().getCon();
        try {
            String sql = "SELECT * FROM aphrodis.technicien_block WHERE tel = \'" + phoneNum + "\'";
            rs = conn.createStatement().executeQuery(sql);
            while (rs.next()) {
                technicien = new Technicien(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4));
            }
    ...
    public int getTechnicienCount() {
        ...
        Connection conn = new ConnectionInit().getCon();
        try {
            String sql = "SELECT COUNT(t.id) FROM technicien_block t";
            rs = conn.createStatement().executeQuery(sql);
            rs.next();
            count = rs.getInt(1);
            return count;
    ...
    public Technicien getTechnicienById(int id) {
        String sql = "SELECT * FROM aphrodis.technicien_block WHERE id = \'" + id + "\'";
        ResultSet rs = null;
        Connection conn = new ConnectionInit().getCon();
        try {
            rs = conn.createStatement().executeQuery(sql);
